...
class FileIO
{
  ...
  public static function upload3($field, $folder, $type, $allowedType = NULL , $size)
  {
    $CI = &get_instance();
    $CI->load->library('upload');
    $CI->upload->initialize(array(
      'upload_path' => realpath(APPPATH . '../uploads/' . $folder),
      'allowed_types' => $allowedType != NULL ? $allowedType : 'jpg|jpeg|png|gif|mp4',
      'max_size' =>  $size,
      'encrypt_name' => false,
    ));
    if (!$CI->upload->do_upload($field)) {
  ...
  public static function genericUpload3($field, $allowedType, $oldData = NULL, $data, $size)
  {
    $filename = NULL;
    $filename = !empty($_FILES[$field]['name']) ? FileIO::upload3($field, $field, NULL, $allowedType, $size)['filename'] : (!empty($oldData[$field]) ? $oldData[$field] : NULL);
    $filename = !empty($data["delete_{$field}"]) ? FileIO::delete("/uploads/{$field}/" . $data["delete_{$field}"]) : $filename;
    return $filename;
  }
}
