...
class UserModel extends CI_Model
{
	...
	public function editPhotoMahasiswa($data)
	{
		// if (!empty($data['password'])) $this->db->set('password', md5($data['password']));
		$this->db->set(DataStructure::slice($data, ['photo']));
		$this->db->where('nim', $data['username']);
		$this->db->update('mahasiswaalumni');

		ExceptionHandler::handleDBError($this->db->error(), "ubah Mahasiswa", "Mahasiswa");
