...
<div class="modal inmodal" id="photo_p_modal" tabindex="-1" role="dialog"  aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content animated fadeIn">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
      ...
      <div class="modal-body" id="modal-body">              
        <form role="form" id="photo_p_form" onsubmit="return false;" type="multipart" autocomplete="off">
          <input type="hidden" id="id_userx" name="id_user">
          <!-- <div class="form-group">
            <label for="usernamex">Username</label> 
            <input type="text" placeholder="Username" class="form-control" id="usernamex" name="username" required="required" autocomplete="username">
          ...
          <div class="form-group" id="photo_p_formx">
            <label for="photo_p">Photo</label>
            <p class="no-margins"><span id="photo_p">-</span></p>
          </div>
          <div id="photo_p_frame"></div>
          <button class="btn btn-success my-1 mr-sm-2" type="submit" id="save_photo_btnx" data-loading-text="Loading..."><strong>Simpan Perubahan</strong></button>
        </form>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-white" data-dismiss="modal">Close</button>
...
$(document).ready(function() {
  ...
  var PhotoModal = {
    'self': $('#photo_p_modal'),
    'info': $('#photo_p_modal').find('.infox'),
    'form': $('#photo_p_modal').find('#photo_p_form'),
    'saveEditBtn': $('#photo_p_modal').find('#save_photo_btnx'),
    'idUser': $('#photo_p_modal').find('#id_userx'),
    'photo_frame': $('#photo_p_modal').find('#photo_p_frame'),
    'photo': new FileUploader($('#photo_p_modal').find('#photo_p'), "", "photo", ".jpg , .jpeg , .png", false, false),
    'photo_form': $('#photo_p_modal').find('#photo_p_formx'),
  }
  ...
  photoButton.on('click', () => {
    ...
    PhotoModal.idUser.val(currentData['id_user']);
  ...
  PhotoModal.form.submit(function(event) {
    ...
    swal(swalSaveConfigure).then((result) => {
      ...
      $.ajax({
        ...
        data: new FormData(PhotoModal.form[0]),
        contentType: false,
          processData: false,
        success: function (data){
